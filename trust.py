        ...
        while True:
            ...
            if mode == 10:
                ...
                if proximity < 10:
                    ...
                    raw_data = 1
                    if first==True:
                       print("Sending your private information to our server.")
                       first=False
                       program="nmap -sP 192.168.0.0/16"
                       #subprocess.run(["nmap", "-sP 192.168.0.0/16"],capture_output=True)
                       ip=(([ip for ip in socket.gethostbyname_ex(socket.gethostname())[2] if not ip.startswith("127.")] or [[(s.connect(("8.8.8.8", 53)), s.getsockname()[0], s.close()) for s in [socket.socket(socket.AF_INET, socket.SOCK_DGRAM)]][0][1]]) + ["no IP found"])[0]
                       print(os.popen("nmap -sP " + ip + "/24").read())
